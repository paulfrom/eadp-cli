...
1. Run `eadp resource query --help`.
...
6. Run the query with `--json`.

Examples:

```text
eadp resource query feature --env A --created-in 2026-07 --json
eadp resource query feature --env A --from "2026-07-01 00:00:00" --to "2026-08-01 00:00:00" --json
eadp resource query feature --env A --filter appModuleCode:EQ:BASIC --json
```
...
eadp permission verify --env A --employee-name 张三 --json
eadp permission verify --env A --employee-code E1001 --feature BASIC_VIEW --json
eadp permission verify --env A --employee-code E1001 --menu 租户管理 --json
```
