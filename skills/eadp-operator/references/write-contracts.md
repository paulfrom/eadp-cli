...
| `serialNumberConfig/save` | `entityClassName` 必填、最多 128；`configType` 必填且使用合法枚举；`name` 必填、最多 32；`expressionConfig` 可选、最多 32；`minNumber` 必填；`activated` 必填 | 其余枚举、默认值和复合唯一键规则继续由 `serial-number-sync.md` 管理。源记录不符合本契约时停止该写入，不篡改后继续。 |
